...
public class AddInteraction implements ServerSideAction<AccountDashboardExtension> {
	...
	public ServerSideActionResult<AccountDashboardExtension> execute(AccountDashboardExtension bean,
		...
		if(bean.getAccount() == null) {
			throw new ValidationException(AccountDashboard.accountPropertyName, "Account is required");
		}
		if (bean.getAccount().getInteractionType() == null) {
			throw new ValidationException(
					Binder.createCompoundBinding(AccountDashboard.accountPropertyName, Account.interactionTypePropertyName),
					"Type is required");
		}
		if (bean.getAccount().getInteractionDescription() == null) {
			throw new ValidationException(
					Binder.createCompoundBinding(AccountDashboard.accountPropertyName, Account.interactionDescriptionPropertyName),
